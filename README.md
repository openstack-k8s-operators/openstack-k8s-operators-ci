...
| [CRD change check](.github/workflows/reusable-crd-change-check.yaml) | `workflow_call` | Fails PRs that introduce CRD changes on maintenance branches (`18.0-frX`). Add the `crd-override` label to the PR when a change is intentional. |
